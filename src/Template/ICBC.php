 ...
 */
class ICBC extends BankBase
{
    protected $bankName = '工商银行';
    ...
    protected function pay(): array
    {
        ...
        if ($pos === false) {
            ...
            return $this->back($matches[1], $this->units($matches[2]), $this->bankName, 0, self::PAY, false);
        } else {
            $pattern = '/(\d{4}).*\)(\d+(\.\d{1,2})?)\D+(\d+(\.\d{1,2})?)/';
            $result = preg_match($pattern, $this->message, $matches);
            if ($result !== 1) {
                throw new MatchException("Match failed");
            }
